...
# No inbound access exists to the VM; Azure requires a key, so use a throwaway one.
key="$(mktemp -u)"; ssh-keygen -q -t ed25519 -N "" -f "$key"

log "Deploying agent VM vm-i2-agent-${I2_ENV}-001 into $RG (pool $VNET_AGENT_POOL)"
