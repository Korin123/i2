...
Put your organisation's root certificate here as a `.crt`, `.cer` or `.pem` file (Base-64/PEM format). Files in this folder are git-ignored. The dev container trusts them when it is created; to apply one to a running container, run `bash .devcontainer/azure-tools.sh`.
